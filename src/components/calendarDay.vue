...
<template>
    <div class="border" style="padding: 12px">
        ...
        <div class="grid grid-cols-7">
            <span class="w-10 h-10 hover:bg-gray-400 text-center" v-for="(item, index) in monthDays" :style="{
                background: Props.date.day == item && (index >= currentMonthSection[0] && index < currentMonthSection[1]) ? '#fff000' : 'transparent', color: index < currentMonthSection[0] || index >= currentMonthSection[1] ? 'gray' : 'black'
            }" :key="index">{{ item }}</span>
        </div>
    </div>
</template>
